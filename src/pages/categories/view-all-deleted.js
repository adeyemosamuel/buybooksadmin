...
import { getDeletedCategoriesUrl, undoCategoryDeleteUrl } from '../../networking/external-url';
...
const ViewAllDeleted = (props) => {
  ...
  const getTableData = () => {
      toggleSpinner("show");
      getData(`${getDeletedCategoriesUrl}false`)
      .then((data) => {
          if(data) {
  ...
  const restoreButton = (cell, row) => {
    const buttonClick = () => {
      showConfirmAlertWithCallBack("warning", "This category would be restored", () => buttonAction());
    }

    const buttonAction = () => {
      getData(`${undoCategoryDeleteUrl}${row.id}`)
      .then((data) => {
          if(data) {
              if(data.responseCode == 99) {
              ...
              showAlert("error", "Unable to restore category at the moment. Try again later");
          }
      });
    }
  ...
  return(
    <AppLayout pageName="View All Deleted Categories">
      <Row as="div">
