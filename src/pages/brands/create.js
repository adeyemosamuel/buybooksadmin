...
import {addBrandUrl, getCategoriesWithCustomParamsUrl} from "../../networking/external-url";
...
const Create = (props) => {
    ...
    const submit = async (e) => {
        e.preventDefault();
        toggleSpinner("show");
        let addBrandResponse = await postData(addBrandUrl, state);
        if(addBrandResponse) {
            if(addBrandResponse.responseCode == 99) {
                showAlert("success", "New brand created successfully");
                await Router.push("/brands/view-all");
                setActive({id: 5, subMenuId: 1});
                toggleSpinner("hide");
            }else {
                toggleSpinner("hide");
                showAlert("error", addBrandResponse.errorMessage);
            }
        }else {
            toggleSpinner("hide");
    ...
    return(
        <AppLayout pageName="Create Brand">
            <Row as="div">
